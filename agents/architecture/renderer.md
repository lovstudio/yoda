...
- `src/renderer/app/workspace-runtime-bar.tsx`: passive active-session runtime status plus the single project/global Terminal toggle used by quick actions; closing only hides the drawer, so the next toggle restores its last active scope before considering the routed project; task-local drawers keep their own controls, and runtime actions stay on explicit Agent surfaces
