...
final class ActionViewController: UIViewController, UITextViewDelegate {
  ...
  private func configureView() {
    ...
    subtitleLabel.text = "把截图交给 Yoda Mobile"
    ...
    sharedTextLabel.layoutMargins = UIEdgeInsets(top: 14, left: 14, bottom: 14, right: 14)
    ...
    buttonConfiguration.title = "开始一项新工作"
  ...
  private func finishLoading() {
    ...
    statusLabel.text = "确认后会在 Yoda Mobile 中创建新任务"
    updateSubmitState()
  }
  ...
  @objc private func submit() {
    ...
    let marker = "\(markerPrefix)\(token)|\(imageData == nil ? "text" : "image")"
    ...
    if let sharedText {
      ...
      payload = prompt.isEmpty ? marker : "\(marker)\n\(prompt)"
    }

    var item: [String: Any] = [UTType.plainText.identifier: payload]
    if let imageData {
