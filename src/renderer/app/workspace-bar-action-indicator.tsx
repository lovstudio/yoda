...
/**
 * A compact runtime-bar action is one 14px glyph centred in a fixed slot, so
 * the row should read as an even rhythm. What the eye measures is the gap
 ...
 *
 * Counters and status dots therefore anchor to the glyph box and end flush with
 * its right edge, which keeps every gap in the row identical however many
 * actions carry an indicator. Use these primitives instead of hand-positioning
 * one, or the row drifts back out of rhythm.
 */
...
export const WORKSPACE_BAR_ACTION_COUNT_CLASS =
  'absolute -top-1.5 right-0 inline-flex h-3 min-w-3 items-center justify-center rounded-full bg-background-secondary px-0.5 font-mono text-[8px] leading-none tabular-nums ring-1 ring-border/60 @min-[1441px]:hidden';
...
export const WORKSPACE_BAR_ACTION_DOT_CLASS =
  'absolute -top-1 right-0 size-1.5 rounded-full @min-[1441px]:hidden';
