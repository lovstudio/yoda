...
async function resolveCwd(candidate?: string, requireCwd = false): Promise<string> {
  const cwd = candidate?.trim();
  if (!cwd) {
    if (requireCwd) throw new Error('A project directory is required.');
    return os.homedir();
  }
  try {
...
export class WorkspaceShellService {
  ...
  async start(params: StartWorkspaceShellParams): Promise<{ sessionId: string }> {
    ...
    const cwd = await resolveCwd(params.cwd, params.requireCwd);
