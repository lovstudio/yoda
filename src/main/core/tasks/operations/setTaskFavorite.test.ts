...
import { setTaskFavorite } from './setTaskFavorite';
...
describe('setTaskFavorite', () => {
  ...
  it('persists the independent favorite marker', async () => {
    mocks.limit.mockResolvedValue([{ id: 'task-1' }]);

    await setTaskFavorite('task-1', true);
  ...
  it('rejects unknown tasks without issuing an update', async () => {
    ...
    await expect(setTaskFavorite('missing-task', true)).rejects.toThrow(
